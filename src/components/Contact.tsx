        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16">
          <div>
            ...
            <div className="mt-10 space-y-3">
              {[
                { icon: Instagram, label: "@n.digital", href: "https://instagram.com" },
                { icon: MessageCircle, label: "WhatsApp", href: "https://wa.me/" },
                { icon: Mail, label: "hello@n-digital.com", href: "mailto:hello@n-digital.com" },
          ...
          >
            {sent ? (
              <div className="py-16 text-center">
                ...
                <p className="mt-5 font-display text-xl">{t("contact.sent")}</p>
              </div>
            ) : (
              <>
...
function Field({
  ...
  const cls = "w-full rounded-2xl bg-white/50 dark:bg-white/5 border border-border/60 px-4 py-3.5 text-sm outline-none focus:border-[color:var(--pink-primary)] focus:ring-2 focus:ring-[color:var(--pink-primary)]/20 transition";
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">{label}</span>
      {textarea ? (
        <textarea name={name} rows={5} className={cls} required />
      ) : (
